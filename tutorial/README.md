...
Four notebooks that walk through the whole workflow, in order. Each one is
...
[`basic_usage.py`](basic_usage.py) — the same end-to-end workflow as a plain
script, using a synthetic volume, so it runs with no data at all.
