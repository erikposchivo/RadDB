...
We recommend users and contributors first set up a virtual environment to install RadDB.
...
While not mandatory, utilizing a virtual environment when installing RadDB is recommended.
...
.. code-block:: bash
   ...
   git clone https://github.com/<your-account>/raddb.git
   cd raddb
...
Install the RadDB package in editable mode by executing the following command in the RadDB repository's root:
...
Install the pre-commit hook by executing the following command in the RadDB repository's root:
...
Specific functionalities in RadDB require additional optional dependencies.
To unlock the full functionalities offered by RadDB, it is recommended to install also the packages detailed here below.
