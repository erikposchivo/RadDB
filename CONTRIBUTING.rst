...
Hi! Thanks for taking the time to contribute to RadDB.
...
In this section we explain the steps that each developer must follow to contribute to the RadDB repository.
...
to create the local copy of the RadDB repository, set up the developing environment and the pre-commit hooks.

Once you have have a local copy of the RadDB repository on your machine, you are ready to
...
They are the only ones who accept Pull Requests and push commits directly to the RadDB repository.
...
RadDB tests are written using the third-party `pytest <https://docs.pytest.org>`_ package.

The tests are organized within the ``/raddb/tests`` directory and are structured to comprehensively assess various aspects of the code.
...
Once your code has been uploaded into your RadDB fork, you can create a GitHub Pull Request (PR) to the RadDB main branch.
